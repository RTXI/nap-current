...
g_Na_p::g_Na_p(void) : DefaultGUIModel("Persistent Sodium", ::vars, ::num_vars),
                       seed(13), noise_on(0), vna(55.0), gnap_max(0), 
                       nnap(1200), pmax(1.0), taup(0.15), v_half(-45.9), dv(3.2)
{
	setWhatsThis(
...
void g_Na_p::update(DefaultGUIModel::update_flags_t flag)
{
	switch (flag)
	{
	case INIT:
